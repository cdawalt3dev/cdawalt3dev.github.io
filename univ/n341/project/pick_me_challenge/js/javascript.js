...
/*
 ...
 CREATE DATE: 4 November 2016
 PURPOSE: see pick_me.html, this does the work.
 MODIFICATION HISTORY:
 CHALLENGE MOD:
 ~the "imgStatus" class is filtered out of tag selections (see  $("#btnSelectTag").click  ).
 ~check-box areas are set and cleared by ".html(...)" instead to set up / remove an image of a check-mark.
 */
...
$(document).ready(function(){
	...
	$("#btnSelectTag").click(function(){
		...
		if(strSelector !== null){
			...
			if(ary_qryRef_selection === null || ary_qryRef_selection.length === 0){
				...
				//MODDED: does not allow anything of the ".imgStatus" selector, used by checks.  Would be confusing to try enabling the "checkmark" of a checkmark
				showSelection( ary_qryRef_selection.not(".imgStatus") );
	...
	*/
	function checkOnTable(arg_jqyRef){
		...
		//MODDED: creates an image via an HTML edit instead of showing the checkmark ASCII character.
		//arg_jqyRef.parent().parent().children(":first").text(strCheckmark);
		arg_jqyRef.parent().parent().children(":first").html("<img class=\"imgStatus\" src=\"../../img/ico_testFile.png\">");
	...
	*/
	function clearOnTable(arg_jqyRef){
		...
		//MODDED: clears the HTML contents, as they were adjusted to create a brand new tag in a previous edit.
		//arg_jqyRef.parent().parent().children(":first").text(" ");
		arg_jqyRef.parent().parent().children(":first").html(" ");
