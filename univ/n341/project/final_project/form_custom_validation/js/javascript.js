...
$(document).ready(function(){
	...
	*/
	$(window).load(function(){
		...
		$("#chkStrictTelephone").prop("checked", true);
	...
	*/
	...
	jQuery.validator.addMethod("moneyBoxCheck",
		function(value, element, options) {
			...
			if(value.charAt(0) === "-" || (value.length > 1 && value.charAt(1) === "-") ){
	...
	jQuery.validator.addMethod("customDateCheck",
		function(value, element, options) {
			...
			//deeper test
			//blnTest = /^[\da-zA-Z]{0,12}[\\\ \/\-][\d]{0,4}[\\\ \/\-][\d]{0,12}$/.test( value );
			blnTest = /^[\da-zA-Z]{0,12}[\\\ \/\-][\d]{0,12}[\\\ \/\-][\d]{0,4}$/.test( value );
