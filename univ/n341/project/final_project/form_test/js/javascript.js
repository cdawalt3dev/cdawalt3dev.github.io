...
$(document).ready(function(){
	...
	*/
	...
	//telephone fields are more flexible so that the user may enter anything deemed a "number".  Also allows parenthesis and dashes.  The ideal format, (###) ###-####, is made apparent regardless.
	/*
	$.validator.methods.tel = function( value, element ) {
		//The telephone field will be flexible.  Accept parenthesis and dasthes.  Trust that the user enters a real phone number if they punch anything in.
	...
	jQuery.validator.addMethod("customTelephoneCheck",
		function(value, element, options) {
			...
			if($("#chkStrictTelephone").prop("checked")){
				...
				//if non strict, make the parenthesis optional and allow some spaces.  
				//a deeper test.
				blnTest = /^\(?\ ?[\d]{3,3}\ ?\)?\ ?\-?\ ?[\d]{3,3}\ ?\-?\ ?[\d]{4,4}$/.test( value );
				...
				if(digitCount < 10){
					qryRefElement.data("error", "Under 10 digits.  Includes area code.");
					return false;
				}else if(digitCount > 10){
					qryRefElement.data("error", "Over 10 digits.");
					return false;
				}else{
					//okay.
				}
	...
	*/
	$("#btnSubmit").click(function(){
		...
		*/
		...
		//NOTE: despite the above comments, this route has been discarded.  Instead, simply don't "return false".  The default behavior seems to be handling the PC and mobile devices just fine.
	...
	*/
	function removeSpacesFromInput(arg_qryRef_selection){
		...
		//txtTelephoneNumber.setCustomValidity("");
		
	}
	
	/*
	Purpose: given an input text box, remove all consecutive spaces from it.
	Parameters: (qryRef) input element to remove spaces from.
	Return: none
	*/
...
http://stackoverflow.com/questions/7657824/count-the-number-of-integers-in-a-string
	...
	*/
	function beginsWith(arg_str, arg_check){
		...
		//cut the 1st string to the size of arg_check.
